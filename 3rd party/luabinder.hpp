...
namespace rf
{
	static std::map<lua_State*, std::map<void*,std::string>> name_storage;
	namespace wrapper
	{
		using std::size_t;
		using std::cout;
		...
		public:
				...
				typename enable_if<is_number<T>::value>::type* = 0)
			{
				...
				{
					return static_cast<T>(Id2Int(lua_tostring(L, index)));
				}
		...
		private:
			...
			struct invoker < Ret(*)(Args...), intetgral_sequence<size_t, Ixs...> >
			{
				static int apply(lua_State* L)
				{
					...
					{
						auto a = name_storage.find(L);
						if (a != name_storage.end())
						{
							auto b = a->second.find(f);
							if (b != a->second.end())
							{
								return luaL_error(L, "function \"%s\", error: \"%s\"", b->second.c_str(), e.what());
							}
						}
						push_stack(L, false);
						push_stack(L, e.what());
